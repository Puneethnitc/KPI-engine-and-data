# IMPLEMENTATION HANDOFF — observational event assessment
# Current: daily treated-minus-control gap regression on pre/post indicator with
# HAC uncertainty, exposure checks, pretrend and quiet-window placebo gates.
# Fixed policy: 14 pre/7 post days; 2-3 quiet windows of >=14 days; HAC lag <=7;
# alpha=.05; pre-event shift >=.75 of effect; placebo >=.5 of effect. Weekly
# exposure requires Monday onset, 3 pre weeks and 2 post weeks.
# Next: resolve these policies with confidence.py from one definition and consume
# authorized native-grain inputs. Authorize every design before sensitivity runs;
# verify_sensitivity itself does not perform the pipeline's access checks.
# Check: incomplete windows, contaminated controls, weak exposure, trend and
# placebo failures stay explicit. DiD effects are average per-day changes, not
# total event losses; any cumulative figure must declare its period and units.
...
from kpi_engine.verification.models import (
    CausalVerificationResult, VerificationDesign, VerificationPolicy,
...
class CausalVerifier:
    ...
    @staticmethod
    def resolve_policy(policy: VerificationPolicy | None = None) -> VerificationPolicy:
        resolved = policy or CausalVerifier.default_policy
        if hasattr(resolved, "validate"):
            resolved.validate()
        return resolved

    def verify_sensitivity(self, frame: pd.DataFrame, contract,
                           designs: tuple[VerificationDesign, ...],
                           policy: VerificationPolicy | None = None) -> VerificationSensitivityResult:
        ...
        results = tuple(self.verify(frame, contract, item, policy=resolved_policy) for item in designs)
    ...
    def verify(self, frame: pd.DataFrame, contract, design: VerificationDesign,
               policy: VerificationPolicy | None = None) -> CausalVerificationResult:
        ...
        try:
            ...
            return self._result(design, "UNTESTABLE", "INVALID_DATES", str(error), policy=resolved_policy)
        if (not pre_start < start <= end or len(windows) not in (2, 3)
            ...
            return self._result(design, "UNTESTABLE", "INVALID_DESIGN",
                                ...
                                policy=resolved_policy)
        if (start - pre_start).days < resolved_policy.min_pre_days or (end - start).days + 1 < resolved_policy.min_post_days:
            return self._result(design, "UNTESTABLE", "INSUFFICIENT_WINDOW",
                                f"Need at least {resolved_policy.min_pre_days} pre-intervention and {resolved_policy.min_post_days} post-intervention days",
                                policy=resolved_policy)
        windows = sorted(windows)
        for index, (first, last) in enumerate(windows):
            if (last >= pre_start or first > last or
                    (last - first).days + 1 < resolved_policy.min_placebo_days or
                    (index and first <= windows[index - 1][1])):
                return self._result(design, "UNTESTABLE", "INVALID_PLACEBO_WINDOWS",
                                    f"Quiet windows must be disjoint, at least {resolved_policy.min_placebo_days} days, and before the pre-period",
                                    policy=resolved_policy)
        if "date" not in frame:
            return self._result(design, "UNTESTABLE", "MISSING_DATE", "Outcome data has no date", policy=resolved_policy)
        data = frame.copy()
        ...
        if abs(driver_effect) <= 1e-9:
            return self._result(design, "INCONCLUSIVE", "NO_EXPOSURE_CONTRAST",
                                "The proposed driver did not change relative to the control",
        ...
        if not all(np.isfinite((effect, *ci, slope, *placebo_effects))):
            return self._result(design, "UNTESTABLE", "UNSCORABLE_ESTIMATE",
                                "The effect or its uncertainty could not be estimated",
                                policy=resolved_policy, **evidence)
        pre_shift = float(pre.iloc[-7:].mean() - pre.iloc[:-7].mean())
        pre_event_failed = (effect * pre_shift > 0 and abs(pre_shift) >= resolved_policy.pre_event_shift_ratio * abs(effect)
                            and abs(pre_shift) > 1e-9)
        trend_failed = abs(slope) > 1e-8 and (not np.isfinite(trend_p) or trend_p < resolved_policy.alpha)
        ...
        if trend_failed:
            return self._result(design, "UNTESTABLE", "PRETREND_VIOLATION",
                                "The treated-control gap was already trending before treatment",
                                policy=resolved_policy, **evidence)
        if design.expected_outcome_direction is not None and effect * design.expected_outcome_direction <= 0:
            return self._result(design, "REJECTED", "DIRECTION_MISMATCH",
                                "The estimated outcome change is opposite the declared hypothesis",
                                policy=resolved_policy, **evidence)
        if ci[0] <= 0 <= ci[1]:
            return self._result(design, "INCONCLUSIVE", "CI_INCLUDES_ZERO",
                                "The HAC 95% interval includes no difference-in-differences effect",
                                policy=resolved_policy, **evidence)
        if any(abs(placebo) >= resolved_policy.placebo_effect_threshold * abs(effect) for placebo in placebo_effects):
            return self._result(design, "UNTESTABLE", "PLACEBO_FAILED",
                                "A quiet-window change is too large relative to the event effect",
                                policy=resolved_policy, **evidence)
        evidence["policy"] = resolved_policy
